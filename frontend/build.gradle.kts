...
val yarnInstallApiSource =
    tasks.register<com.github.gradle.node.yarn.task.YarnTask>("yarnInstallApiSource") {
    ...
    tasks.register("patchApiSource") {
        ...
        doLast {
            ...
            packageJsonFile.writeText(
                packageJson
                    .replace(""""@types/node": "12.11.5 - 12.20.42",${System.lineSeparator()}    """, "")
                    .replace(""""@types/node": "12.11.5 - 12.20.42",\n    """, ""),
            )
        }
    }

val yarnBuildApiSource =
    tasks.register<com.github.gradle.node.yarn.task.YarnTask>("yarnBuildApiSource") {
        dependsOn(yarnInstallApiSource)
...
val copyApiDist =
    tasks.register<Copy>("copyApiDist") {
        dependsOn(yarnBuildApiSource)
        from(yarnBuildApiSource.map { it.outputs.files })
        into(apiDistDir)
    }
